...
resource "oci_core_instance" "CreateInstance_cluster2_node" {
  ...
  source_details {
    ...
    # Optional
    display_name = "${var.display_name_cluster2_node}${count.index}"
    create_vnic_details {
  ...
  provisioner "file" {
    source      = "./module-instance-cluster2/worker-node/scripts/"
    destination = "/tmp/"
